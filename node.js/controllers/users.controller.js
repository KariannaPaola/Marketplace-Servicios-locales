...
export const registerUser = async (req, res) => {
  const {name, lastname, email, phone_number, password, password_repeat} =req.body;
...
export const loginUser = async (req, res) => {
  ...
  try{
    ...
    const token= jwt.sign({ id: user._id, email: user.email, role: user.user_type }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN });
    res.json({ token });
  } catch (err) {
...
export const protectUser = async (req, res) => {
  res.json({ message:'acceso permitido', id: req.user.id, email: req.user.email, role: req.user.role });
}
