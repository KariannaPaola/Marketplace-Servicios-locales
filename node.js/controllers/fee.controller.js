...
export const listAllFees= async (req, res) => {
  ...
  try{
    ...
    const filter={};
    if (status && !["pendiente", "verificado", "rechazado"].includes(status)) {
...
export const verifyFee= async (req, res) => {
  ...
  try{
    ...
    if (!action) {
      return res.status(400).json({ message: "debes ingresar una accion" });
    }
    if (action !=="verificado" && action !=="rechazado" ) {
      return res.status(400).json({ message: "debes ingresar una accion valida" });
    }
    ...
    if (!fee) return res.status(404).json({message:'tarifa no encontrada'});
      ...
      return res.status(200).json({
        message: 
        action==="verificado" 
        ?"referencia de pago de tarifa verificada con exito"
        :"referencia de pago de tarifa rechazada"
    }); 
  } catch (error) {
    res.status(500).json({ 
    message: "Error al verificar tarifa", 
    error: error.message || error.toString() 
    });
  }
}
